...
if($token!="")
{
	...
	$q=$mysql->query("SELECT q.*,p.pg paket_pg,p.essay paket_essay,p.complex paket_complex,m.alamat,m.foto,m.asal_instansi FROM app_quiz_done q LEFT JOIN app_quiz_done_paket p ON p.quiz_done_id=q.id LEFT JOIN quiz_member m on m.id=q.member_id WHERE q.token='$token' AND q.is_done=1");
