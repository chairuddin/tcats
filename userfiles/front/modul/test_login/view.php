<?php
$a=send_email('roemly@gmail.com','testing saja','ini adalah konten uji coba','noreply@bizkit.id',"noreply@bizkit.id");
var_dump($a);
die('aaaaa');
/*SETTING POST*/
$_POST['submit']=1;
$_POST['username']='SMA1-0003';
$_POST['jadwal_token']='BDWGZ';
...
if($_POST['mulai_ujian'])
{
	...
	$is_expired=false;
		...
		$r_json_soal=get_soal_json($data_quiz['id']);
		...
		"
		INSERT INTO 
		quiz_done (member_id,token,member_code,member_class,member_fullname,quiz_id,schedule_id,quiz_duration,quiz_title_id,quiz_code,start_time,start_time_real,acak,acak_pilihan,ip_address,score_master,kkm,browser_key,is_listening)
		SELECT * FROM(SELECT '".$data_member['id']."' A,'".$token_asli."' B,'".$data_member['username']."' C,'".$data_member['class']."' D,'".addslashes($data_member['fullname'])."' E,'".$data_quiz['id']."' F,'".$data_schedule['id']."' G,'".$data_quiz['duration']."' H,'".$data_quiz['title_id']."' I,'".$data_quiz['code']."' J,'$start_time' K,'$start_time_real' L,'$acak' M,'$acak_pilihan' N,'".$_SERVER['REMOTE_ADDR']."' O,".$data_quiz['score']." P,".$data_quiz['kkm']." Q,'$browser_key' R,'".$data_quiz['is_listening']."' S) a
		WHERE 
...
if($_POST['submit'])
{
	...
	$q=$mysql->query("
	SELECT s.* FROM quiz_schedule s WHERE lower(s.token)='".strtolower($jadwal_token)."' 
	AND '$hariini_long' <= s.tanggal_expired
	AND (find_in_set('".$data_member['class']."',s.allow_class))
	AND s.id NOT IN(SELECT schedule_id  FROM quiz_done 	WHERE member_id='".$data_member['id']."' and schedule_id=s.id and is_done<>3) 
	");
	...
	if($q and $mysql->numrows($q)>0){
		...
		$r_msg_warning[]="Jadwal tidak tersedia";
	}
	
	
	
	$data_quiz=get_master_soal_json($kode_soal);
	if($jadwal AND count($data_quiz)>0)
	{
		$issoal=true;
		$is_expired=true;
		if($issoal)
		{
			...
			if($q and $mysql->numrows($q)>0){
			list($token,$t_start_time,$t_check_point,$t_quiz_duration)=$mysql->row($q);
	...
	{
		$r_msg_warning[].="Pastikan token yang dimasukkan benar!";
	}
...
<div class="info-danger text-center bg-danger">
		...
		if(count($r_msg_warning)>0){
			...
			echo "<span class='error_message'><ul><li>".join("</li><li>",$r_msg_warning)."</li></ul></span>";
		}
