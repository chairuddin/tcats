...
$run_js_script2="";
$url_back = "";
$url_back = fronturl("app_quiz_login");
$this_time=date("Y-m-d H:i:s");

$_POST=$_GET;
//die("SELECT username FROM quiz_member WHERE token='".cleanInput($_POST['token'])."'");
$_POST['username']=$mysql->get1value("SELECT username FROM quiz_member WHERE token='".cleanInput($_POST['token'])."'");

//$_POST['jadwal_token']='FNQXN';
//ed49cccacd0617029aacc961a604f51a
//31
//ed49cccacd0617029aacc961a604f51a
//32
...
if($_POST['mulai_ujian'])
{
	...
	//KONVERSI KE MD5
	$token_asli=md5($_POST['token']."-".$course_material_id);
    ...
    $q=$mysql->query("
	SELECT q.* FROM quiz_master q LEFT JOIN app_quiz_done d ON (q.id=d.quiz_id and d.member_id='".$data_member['id']."') WHERE
	md5(md5(md5(q.id)))='$quiz_id_md5'
	/*AND ISNULL(d.member_code) */
	");

	/*AND ISNULL(d.member_code) <= dimatikan nyoba nyoba aja sebenarnya*/ 
	...
	$is_expired=false;
		...
		if(count($_COOKIE)>0)
		{
			foreach($_COOKIE as $i =>$v)
			{
				...
				if($i!="PHPSESSID" and $i!="STDSESSID" and $i!='qr_token'){
					setcookie($i,"",-1,"/");
				}
		...
		header("location:".fronturl("app_quiz_start"));
		exit();
		}
...
if($token!=""){
	...
	$q=$mysql->query("
		SELECT d.is_done,d.token dtoken,d.start_time dstart_time,d.check_point dcheck_point,d.quiz_duration dquiz_duration
		FROM   
			app_quiz_done d
		WHERE 
	    d.token='$token'
...
if($_POST['submit'])
{
	...
	if($valid){
		...
		FROM
			quiz_master q INNER JOIN app_quiz_done d ON d.member_id='".$data_member['id']."' WHERE d.course_material_id=$course_material_id
		";
		...
		if($q and $mysql->numrows($q)>0) {
			...
			if($data_schedule['is_done']==="0")
			{
				...
				if($selisih>1){
					...
					$q=$mysql->query("UPDATE app_quiz_done SET is_done=3 WHERE id=".$data_schedule['ujian_id']);
					$q=$mysql->query($sql);
					$data_schedule=$mysql->assoc($q);
			...
			if($data_schedule['is_done']=="1"){
			    header("location:".fronturl("app_quiz_done/".$data_schedule['dtoken']));
			    exit();
				$valid=false;
	...
	if($valid AND count($data_quiz)>0)
	{
		...
		if($issoal)
		{
			...
			if($valid) {
				...
				$token_uniq=md5(uniqid().time().$username);
				...
				$q=$mysql->query("UPDATE app_quiz_done SET is_done=0,start_time='$this_time',start_time_real='$this_time',check_point='$this_time',quiz_duration='$t_quiz_left_duration',token='$token' WHERE id=".$data_schedule['ujian_id']);
...
<?php
$ada_ujian_aktif=0;
//var_dump("($ismember and $issoal) OR $ada_ujian_aktif");
...
?>
<div class="form-baris form-group" style="display:none;">
...
?>
	...
	if(!$is_expired)
	{
		...
		if($ada_ujian_aktif){
		echo "<input type=\"button\"  class=\"quiz_button\" name=\"mulai_ujian\" onclick=\"window.location.href='".fronturl("app_quiz_start")."'\" value=\"Lanjutkan ujian!!\"/>";
		}else{
...
<p class="text-center">Ujian tidak tersedia</p>
<!--
<h1>User Login</h1>
