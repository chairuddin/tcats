...
if($token!="")
{
	$q=$mysql->query("SELECT * FROM quiz_done WHERE token='$token' AND is_done=1");
				...
				foreach($pilihan as $i=>$pil)
				{
					...
					?>
						<div class="kolom_input" <?php echo $pil==$v['answer']?'style="border:2px solid green;"':'';?>>		
							<input mark-choice="mark-value" dionclick="<?php echo $no<$total?"tampilkan_soal_delay('".($no+1)."',$web_config_navigasi_soal)":""; ?>" class="radio_pilihan icheck-me"  <?php echo $checked;?> id="<?php echo $pil.$no;?>" type="radio" meta-urutan="<?php echo $no;?>" meta-name="soal<?php echo $no;?>" meta-label="label_<?php echo $pil.$no;?>" name="soal_<?php echo $no_sss;?>" value="<?php echo $pil;?>" />
	...
	foreach($r_json_soal['soal_essay'] as $v)
	{
		...
		<div class='wrap_soal row' id="wrap_soal<?php echo $no?>">
			...
			* */
			...
			<div>
				Jawaban:<br/><?php echo $r_jawab_essay[$v['id']]['answer'];?>
				</div>
			<br/>
			</div>
		</div>
...
//		  alert('load offline');
	...
	if(bolong)
	{
		...
		Swal.fire({
		  ...
		  text: "Apakah anda yakin tetap mau mengakhiri ujian anda? Masih ada "+bolong+" soal pilihan ganda yang belum dijawab!!! ",
		  icon: 'warning',
	...
	Swal.fire({
		  title: 'Apakah anda yakin?',
		  text: "Masih ada waktu untuk mengecek jawaban anda! ",
		  icon: 'warning',
		  showCancelButton: true,
