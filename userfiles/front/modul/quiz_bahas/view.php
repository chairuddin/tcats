<?php
$bahas_soal=1;
$data_lab=$mysql->query_data("SELECT id,title,content FROM laboratorium ORDER BY title ");
...
if($token!="")
{
	...
	$q=$mysql->query("SELECT q.*,p.pg paket_pg,p.essay paket_essay,p.complex paket_complex,m.alamat,m.foto,m.asal_instansi FROM quiz_done q LEFT JOIN quiz_done_paket p ON p.quiz_done_id=q.id LEFT JOIN quiz_member m on m.id=q.member_id WHERE q.token='$token' AND q.is_done=1");
	if($q and $mysql->numrows($q)>0)
	{
		$member_data=$mysql->assoc($q);
		$now=date("Y-m-d H:i:s");
		$quiz_log=$mysql->query(" UPDATE quiz_done SET bahas_log='$now' WHERE token='$token' AND is_done=1 AND (bahas_log='0000-00-00 00:00:00' OR ISNULL(bahas_log)) ");
		$bahas_log=$mysql->get1value("SELECT  bahas_log FROM quiz_done WHERE token='$token' AND is_done=1 ");
		$diff = strtotime($now) - strtotime($bahas_log);
		$selisih=abs(round($diff / 86400));
		if($selisih>5) {
			sweetalert2("warning",'Pembahasan sudah expired',fronturl('siswa/riwayat'));
		}
...
$member_foto='';
if($member_data['foto']) {
	if(file_exists($path_images_large.'/'.$member_data['foto'])) {
		$member_foto=$url_images_large.'/'.$member_data['foto'];
	} else {
		$member_foto='https://statinfer.com/wp-content/uploads/dummy-user.png';
	}
}
...
<div id="left-panel">
	<div id="show-left-panel">
		<div class="show-jawaban"><i class="glyphicon glyphicon-user"></i></div>
		<div class="hide-jawaban"><i class="glyphicon glyphicon-user"></i></div>
	</div>
	<div class="thumbnail-panel">
		<img class="photo-user" src="<?php echo $member_foto;?>">
		<div class="data-user">
			<label>Nama Peserta</label>
			<div class="content-user"><?php echo $member_data['member_fullname']; ?></div> 
		</div>
		<div class="data-user">
			<label>Paket Dipilih</label>
			<div class="content-user"><?php echo $member_data['member_class']; ?></div>
		</div>
		<div class="data-user">
			<label>Asal Institusi</label>
			<div class="content-user"><?php echo $member_data['asal_instansi']; ?></div>
		</div>
		<div class="data-user">
			<label>Alamat</label>
			<div class="content-user"><?php echo $member_data['alamat']; ?></div>
		</div>
	</div>
</div>
<div id="right-panel">

	<div id="show-right-panel">
		<div class="show-jawaban"><i class="glyphicon glyphicon-info-sign"></i></div>
		<div class="hide-jawaban"><i class="glyphicon glyphicon-info-sign"></i></div>
	</div>
<?php
$waktu_ujian_show=<<<END
<div id="divwaktu_ujian" class="sisa-waktu">
...
?>
	...
	<div class="thumbnail-panel" id="terjawab">
		<div class="title-panel">Soal Terjawab</div>
		<div id="soal-terjawab" class="number-block">0</div>
	</div>
	<div class="thumbnail-panel" id="belumdijawab">
		<div class="title-panel">Belum Terjawab</div>
		<div id="soal-tidak-terjawab" class="number-block">0</div>
	</div>
	<div class="thumbnail-panel" id="raguragu">
		<div class="title-panel">Ragu-Ragu</div>
		<div id="soal-ragu" class="number-block">0</div>
	</div>
	<div class="thumbnail-panel" id="lab-button">
		<a href="#" data-target="#modal-laboratorium"  data-toggle="modal">Lihat Data Laboratorium</a>
	</div>
</div>
	...
	foreach($r_acak as $index)
	{
		...
		<div class='wrap_soal' id="wrap_soal<?php echo $no?>" berikut="<?php echo $button_berikut!=""?($no+1):"";?>">
			<div id="ukuran-soal">
				...
				<?php
				//echo $button_sebelum;
				echo "<div class='nomor-soal'>Pebahasan Soal No<span>$no</span></div>";	 
				//echo $button_berikut;
				?>
				</div>
			</div>
...
<div class="container">
<div id="block-list-jawaban-line">
	<div id="show-hide-jawaban">
		<div class="show-jawaban"><i class="glyphicon glyphicon-chevron-left"></i></div>
	...
	<input type="button" id="tombol_selesai" class="btn btn-maroon"  name="akhiri" value="Keluar" onclick="document.location.href='<?php echo fronturl('siswa');?>'">
</div>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title" style="line-height:10px;">Data Laboratorium</h4>
            ...
            <div class="modal-body">
              ...
              <?php
              if(count($data_lab)>0) {
				foreach($data_lab as $ilab => $lab) {
					echo '<li><a href="#" data-target="#modal-laboratorium-'.$lab['id'].'"  data-toggle="modal">'.$lab['title'].'</a></li>';
				}
			  }
              ?>
              </ul>
 ...
 <?php
  if(count($data_lab)>0) {
	foreach($data_lab as $ilab => $lab) {
		$id_lab=$lab['id'];
		$judul_lab=$lab['title'];
		$konten_lab=$lab['content'];
		echo <<<END
		<div class="modal fade" tabindex="-1"  id="modal-laboratorium-$id_lab">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title" style="line-height:10px;">$judul_lab</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
...
.modal-body > table > tr:first-child {
  color:black;
} 
.modal-body {
 font-size:9pt;
...
//		  alert('load offline');
	...
	function cek_jumlah_jawaban() {
	 ...
	 var ragu = $('input:checkbox:checked').length;
  ...
  return obj;
	...
	function update_summary() {
		...
		obj=cek_jumlah_jawaban();
		bolong=obj.bolong;
		terjawab=obj.terjawab;
		ragu=obj.ragu;
		$("#soal-terjawab").html(terjawab);
		$("#soal-tidak-terjawab").html(bolong);
		$("#soal-ragu").html(ragu);
