...
if($_GET['schedule_id']!="" AND $_GET['class']!="")
{
...
$sql="SELECT d.*,c.answer answer_complex FROM quiz_done d LEFT JOIN quiz_done_complex c ON d.id=c.id_quiz_done WHERE d.is_done=1 AND d.schedule_id='".$_GET['schedule_id']."'";
if($_GET['class']!="" AND $_GET['class']!="ALL"){
	$sql.=" AND d.member_class='".$_GET['class']."'";
}
...
$json_answer=join("",$r_answer);	
	...
	$objPHPExcel->setActiveSheetIndex(0)
        ...
        $nilai_kd=array();
        $kkm_kd=array();
        $q_nilai_kd = $mysql->query("SELECT id_quiz_kd,score,kkm FROM quiz_done_kd WHERE id_quiz_done=".$d['id']);    
...
$objWorkSheet->setCellValue('A1', 'ANALISIS JAWABAN '.$dschedule['title_id']);
...
$objWorkSheet->setCellValue('A3', '')
		   ...
		   ->setCellValue('D3', '');
/*ambil persentase*/		   
$data_persen=persentase_jawaban($_GET['schedule_id'],$_GET['class']);
$data_persen_complex=persentase_jawaban_complex($_GET['schedule_id'],$_GET['class']);
...
$schedule_id=cleanInput($_GET['schedule_id']);	
list($dschedule)=$mysql->query_data("SELECT * FROM quiz_schedule WHERE id=$schedule_id");
$jumlah_soal_essay = $mysql->get1value("SELECT count(id) FROM quiz_essay WHERE quiz_id = ".$dschedule['quiz_id']);
$master_essay = $mysql->query_data("SELECT id FROM quiz_essay WHERE quiz_id=".$dschedule['quiz_id']." ORDER BY id ");
...
if($r and $mysql->numrows($r)) {
	...
	while($d=$mysql->assoc($r)) {
		...
		$r_kolom=kolom_excel($jumlah_soal_essay,$mulai="D");
		$q_essay=$mysql->query("SELECT answer,id_quiz_essay FROM quiz_done_essay WHERE id_quiz_done ='".$d['id']."' ");
		$isian_essay = array();
