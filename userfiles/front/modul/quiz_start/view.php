...
if($token!="")
{
	$q=$mysql->query("SELECT q.*,p.pg paket_pg,p.essay paket_essay,p.complex paket_complex,m.alamat,m.foto,m.asal_instansi FROM quiz_done q LEFT JOIN quiz_done_paket p ON p.quiz_done_id=q.id LEFT JOIN quiz_member m on m.id=q.member_id WHERE q.token='$token' AND q.is_done=0");
...
?>
<link href="<<<TEMPLATE_URL>>>/css/ujian.css?1=6" rel="stylesheet">
...
<div id="block-list-jawaban-line">
	...
	<input type="button" id="tombol_selesai" class="btn btn-maroon"  name="akhiri" value="Selesai Ujian" onclick="return cek_jawaban();">
</div>
...
$(document).ready(function(){
	...
	$(".kolom_input>input").change(function(){
		...
		type_soal=$(this).attr("meta-type");
		nama_soal=$(this).attr("meta-name");
		nama_input=$(this).attr("name");
		...
		if(type_soal=='pilihan_ganda') {
			...
			var checked = 0;
			$("input[name='"+nama_input+"']:checked").each(function ()
			{
				checked++;
			});
			if(checked==0) {
	...
	$(".ragu-button").click(function(){
		id=$(this).attr('id');
		nomor_soal=$(this).attr('nomor-soal');
		if($(this).is(':checked')){
			$("#check_soal"+nomor_soal).addClass("ragu");
		} else {
			$("#check_soal"+nomor_soal).removeClass("ragu");
		}
