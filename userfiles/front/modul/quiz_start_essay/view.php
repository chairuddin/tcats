...
////////////////ESSAY
$q=$mysql->query("SELECT qe.id,qe.question,qew.answer FROM quiz_essay qe LEFT JOIN quiz_essay_answer qew  ON (qe.id = qew.essay_id AND qew.done_id='".$member_data['id']."') WHERE qe.quiz_id='".$member_data['quiz_id']."' ORDER BY qe.id");
$ada_essay=0;
if($q and $mysql->numrows($q)>0)
{
	$ada_essay=1;
	...
	while($v=$mysql->assoc($q))
	{
		...
		<div class='wrap_soal row' id="wrap_soal<?php echo $no?>">
			...
			<textarea class="jawab_essay" data-id="<?php echo $v['id'];?>" id="essay_<?php echo $no_sss;?>" name="essay_<?php echo $no_sss;?>"  rows="6" style="width:100%;padding:15px;"><?php echo $v['answer']?></textarea>	
			</div>
			<div class='navigation_button'>
			<?php
			echo $button_sebelum;
...
echo <<<END
	<div class="col-md-4"><img class="logo-instansi-footer" src="$logo_instansi" alt="$web_config_name" title="$web_config_name" /></div>
...
var deadline = new Date(Date.parse(new Date()) + ($quiz_left_duration * 1000));
	...
	{
		if(urutan_cek_valid==$last_digit){
			quiz_token=getCookie("quiz_token");
			if(quiz_token==""){
			document.location.href='$url_login';
			}else{
		...
		//alert(urutan_cek_valid+" = "+$last_digit);
		urutan_cek_valid++;
		if(urutan_cek_valid==10){urutan_cek_valid=0;}
		t = setTimeout(function(){isujianvalid()},2000);
	}
	function autosave(first)
	{
		if(urutan_auto_save==$last_digit){	
			if(first==1 || save_now==1 || getCookie("save_now")==1){
		...
		//alert(urutan_auto_save+" = "+$last_digit);
		urutan_auto_save++;
		if(urutan_auto_save==10){urutan_auto_save=0;}
		t = setTimeout(function(){autosave(first)},1000);
	}	
	...
	function countdown()
	{
		...
		else
		{
			...
			document.getElementById("otomatis").value=1;
			tsub = setTimeout(function(){
	...
	{
	setCookie("soal_ke",nomor,1);
	setTimeout(function(){move_delay(nomor)},700);
	}
