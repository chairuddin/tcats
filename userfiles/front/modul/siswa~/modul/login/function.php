...
if($_POST['do_login']==1) {
	...
	$q=$mysql->query("SELECT id,username,class,jurusan,ruang,fullname,md5(password) password FROM quiz_member WHERE (md5(username)='$username' OR md5(email)='$username' ) AND password='$password' AND status=1");
	$ismember=false;
