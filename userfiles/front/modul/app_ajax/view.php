		...
		if($q and $mysql->numrows($q)>0)
		{
			...
			if($member_data['custom_score']==0) {
				...
				if(count($r_json_soal['soal_complex'])>0)
				{
					...
					$insert_complex=$mysql->query("INSERT INTO app_quiz_done_complex SET id_quiz_done='".$member_data['id']."',answer='".$json_encode_answer."',benar=$benar,salah=$salah,tidak_jawab=$tidak_jawab ON DUPLICATE KEY UPDATE answer='".$json_encode_answer."',benar=$benar,salah=$salah,tidak_jawab=$tidak_jawab ");
			...
			if($member_data['custom_score']==0) {
				...
				if($jumlah_essay>0)
				{
					...
					$benar_essay=$mysql->get1value("SELECT count(id) FROM app_quiz_done_essay WHERE score>0 AND id_quiz_done=".$member_data['id']);
					$benar+=$benar_essay;
			...
			if ($member_data['custom_score']==1){				
				...
				if( $qkd and $mysql->numrows($qkd)>0 ) {
					...
					while($dkd = $mysql->assoc($qkd)) {
						...
						$sql_kd = 'INSERT INTO app_quiz_done_kd SET '.join(",",$score_kd_insert).' ON DUPLICATE KEY UPDATE '.join(",",$score_kd_update);
			...
			if($_GET['akhiri_ujian']==1){
				...
				end_time='$hari_ini',
				is_done=1 ";
				$score_essay=$mysql->get1value("SELECT sum(score) FROM app_quiz_done_essay WHERE id_quiz_done=".$member_data['id']);
...
function akhiri_ujian($token)
{
	...
	if($token!="")
	{
		...
		$q=$mysql->query("SELECT id,quiz_id,acak,score_master FROM app_quiz_done WHERE token='$token' and is_done=0");
		if($q and $mysql->numrows($q)>0)
		{
			ob_start();
			$member_data=$mysql->assoc($q);
			...
			/*UPDATE NILAI*/
			$hari_ini=date("Y-m-d H:i:s");
			$q=$mysql->query("UPDATE app_quiz_done SET 
			check_point='$hari_ini',
			end_time='$hari_ini',
			score_essay='$score_essay',
			is_done=1 
			...
			$q=$mysql->query("SELECT id,quiz_id,acak,score_master FROM app_quiz_done WHERE token='$token' and is_done=1");
			if($q and $mysql->numrows($q)>0){
				echo 1;
			}else{
				echo 0;	
...
if($action=="save_essay"){
	...
	$answer=$_POST['answer'];
	$token=$_POST['token'];
	$done_id=$mysql->get1value("SELECT id FROM app_quiz_done WHERE token='$token' and is_done=0");
	$essay_terpilih=$mysql->get1value("SELECT essay from app_quiz_done_paket WHERE quiz_done_id=$done_id ");
	...
	$q=$mysql->query("
	INSERT INTO app_quiz_done_essay 
	SET 
...
if($action=="timeout_checking"){
	...
	$q=$mysql->query("SELECT start_time,quiz_duration FROM app_quiz_done WHERE token='$token' AND is_done=0");
	if($q and $mysql->numrows($q)>0)
	{
