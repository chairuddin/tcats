...
if($token!="")
{
	$q=$mysql->query("SELECT q.*,p.pg paket_pg,p.essay paket_essay FROM quiz_done q LEFT JOIN quiz_done_paket p ON p.quiz_done_id=q.id WHERE q.token='$token' AND q.is_done=0");
...
';
/*
echo "<span><b>Nama:</b>".$member_data['member_fullname']."</span>";
echo "<span><b>Kelas:</b>".$member_data['member_class']."</span>";
echo "<span><b>Soal:</b>[".$member_data['quiz_code'];
...
if(count($r_json_soal['soal_ganda'])>0)
{
	...
	foreach($r_acak as $index)
	{
		...
		if($no>1)
		{
		$button_sebelum='<a class="prev-page btn btn-default" href="#divbiodata" onclick="tampilkan_soal('.($no-1).')"><i class="glyphicon glyphicon-chevron-left"></i> Soal Sebelumnya</a>';
		}
		else
		{
		$button_sebelum="";	
		...
		if($no<$total)
		{
		$button_berikut='<a  class="next-page btn btn-primary" href="#divbiodata"  onclick="tampilkan_soal('.($no+1).')">Soal Berikutnya <i class="glyphicon glyphicon-chevron-right"></i></a>';
		}
		else
		{
		$button_berikut="";
		...
		if(count($r_json_soal['soal_essay'])>0){
			$button_berikut='<a  class="next-page btn btn-primary" href="#divbiodata"  onclick="tampilkan_soal('.($no+1).')"><i class="glyphicon glyphicon-chevron-right"></i></a>';	
		}
	...
	for($i=1;$i<=$total;$i++)
	{
		...
		$panel_answer.="<li><span class='checked_answer $tandai button-list-jawaban'  onclick='tampilkan_soal($i)' id='check_soal$i'>$i<span class='check_abc' id='answer_soal$i'></span></span></li>";
...
if(count($r_json_soal['soal_essay'])>0)
{
	...
	foreach($r_acak_essay as $index)
	{
		...
		<div class='wrap_soal row' id="wrap_soal<?php echo $no?>">
			...
			<?php
			echo $button_sebelum;
			echo "<div class='nomor-soal'>Soal No<span>".$no_essay."</span></div>";	
			echo $button_berikut;
			?>
			</div>
			</div>
	...
	foreach($r_no_essay as $x => $i)
	{
		...
		$panel_answer_essay.="<li><span class='checked_answer button-list-jawaban $tandai'  onclick='tampilkan_soal($i)' id='check_soal$i'>".($x+1)."</span></li>";
...
<div id="footer-ujian">
	...
	<div class="jawaban-ragu">
		<label for="ragu"><input type="checkbox" name="ragu" id="ragu" value="ragu" /> Ragu-Ragu</label>
	</div>
...
<div id="block-list-jawaban">
	...
	<div class="list-jawaban-line">
		...
		<div class="cek_pilihan_essay">
			<h1>Soal Uraian</h1>			
			<ul><?php echo $panel_answer_essay;?></ul>
		</div>
		<?php
		}
		?>
...
function initializeClock(id, endtime,init_start) {
  ...
  var t = getTimeRemaining(endtime);
	...
	if(t.days<=0 && t.hours<=0){document.getElementById("clockhours").style.visibility="hidden";}
	if(t.days<=0 && t.hours<=0 && t.minutes<=0){document.getElementById("clockminutes").style.visibility="hidden";}	
