<?php
function url_back_login()
{
	global $web_config_mode_login;
	if ($web_config_mode_login == 1) {
		return fronturl("siswa");
	} else {
		return fronturl("quiz_login");
	}
}
...
function send_email($emailto, $subject, $message, $emailfrom = "", $namefrom = "")
{
	...
	$mysql->query("INSERT INTO log(log) values('$message')");


	if ($emailfrom == "") {
		$emailfrom = $config_email_sender;
	}
	if ($namefrom == "") {
		$namefrom = $config_email_name;
	}
