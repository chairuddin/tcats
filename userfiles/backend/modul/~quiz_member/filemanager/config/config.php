...
if($_SESSION['s_level']==0){
	$direktory_guru_1="/".$_SESSION['s_username'];
	$direktory_guru_2=$_SESSION['s_username']."/";
...
$dir_pusat=$config['userdir']."file/".($config['subdir']!=""?$config['subdir']:"");
$dir_member=$dir_pusat."/quiz_member";

	if(file_exists($dir_member)==false){
		mkdir($dir_member,0755);		
	}
	
	if(file_exists($dir_member."/source")==false){
	mkdir($dir_member."/source",0755);
	}
	if(file_exists($dir_member."/thumbs")==false){
	mkdir($dir_member."/thumbs",0755);
	}
	$image_max_width=150;
	$image_max_height=150;
	$upload_dir ="/userfiles/file/".$_d['dir']."/quiz_member/source/"; // path from base_url to base of upload folder (with start and final /)
	$current_path ="../../../../file/".$_d['dir']."/quiz_member/source/"; // relative path from filemanager folder to upload folder (with final /)
	$thumbs_base_path = "../../../../file/".$_d['dir']."/quiz_member/thumbs/"; // relative path from filemanager folder to thumbs folder (with final /)
