...
if($action=="uploadwordx" AND  $_POST['form']=='uploadwordx'){ 
	$new_filename=change_filename($_FILES['file_wordx']['name'],$_GET['id_soal']);
	$destination=$config['userdir'].'backend/modul/quiz_upload_wordx/backup/'.$new_filename;
	//$filename=$_FILES['file_wordx']['name'];
	
	
	$filename=$new_filename;
	$upload=move_uploaded_file($_FILES['file_wordx']['tmp_name'], $destination);
	if (!$upload) {
	    	die($destination);
	...
	if($word_parser==2) {	
		$convert=convert_to_html($new_filename);	
	}
...
//delete_directory("$this_path/$html_folder/$quiz_folder");	
} else {
...
if ($word_parser==2 or ($word_parser==1 and ($index = $zip->locateName("$htmlname")) !== false))  {
	...
	foreach ($xpath->query($query_path) as $table) {
				...
				foreach ($imgs as $i => $img) 
				{
					...
					$src = $img->getAttribute('src');
					if($word_parser==2) {
						$path_file="$this_path/$html_folder/$quiz_folder/$file";
						copy($path_file,DIR_IMAGES."/source/$directory_soal_2{$uniqid}{$file}");
					} else {
						...
						$a=file_put_contents(DIR_IMAGES."/source/$directory_soal_2{$uniqid}{$file}",$image); 
	...
	{	
		...
		if($_POST['timpa']==1){
		$timpa_soal=$mysql->query("DELETE FROM quiz_complex WHERE quiz_id='$id_soal'");
		}
		foreach($r_komplek as $ix => $d)
		{
		
		$nomor=trim($ix);	
		${"model"}=1;
		
		if($A_K=="-" AND $B_K=="-" AND $C_K=="-" AND $D_K=="-" AND $E_K=="-"){
		${"model"}=0;	
		}
		...
		foreach($pilihan_komplek as $pil_komplek) {
			...
			if(in_array($pil_komplek,array('F_K','G_K','H_K','I_K','J_K'))) {
				$d[$pil_komplek]=$d[$pil_komplek]==''?'-':$d[$pil_komplek];
			}
			${"$pil_komplek"}=addslashes($d[$pil_komplek]);
		}

		${"answer"}=cleanInput(strip_tags($d["KUNCI_K"]),'alpha');
		${"question"}=addslashes($d["PERTANYAAN_K"]);
			
		${"id_soal"}=cleanInput($id_soal);
		
		$sql_r=array();
		$r_exist=$mysql->query("SELECT id FROM quiz_complex WHERE quiz_id='".${"id_soal"}."' AND urutan='".$nomor."'");
		if($mysql->numrows($r_exist)>0){
			$sql="
			UPDATE quiz_complex SET ";
			
			$sql_r[]="question='".${"question"}."'";	
			foreach($pilihan_komplek as $pil_kom) {
				$pil=cleanK($pil_kom);
				$sql_r[]="$pil='".${"$pil_kom"}."'";	
			}
			$sql_r[]="answer='".${"answer"}."'";	
