	...
	{	
		...
		//	$value.='/*!40000 ALTER TABLE `'.$table.'` DISABLE KEYS */;' . MSB_NL;
			...
			{	$values = '';
				...
				{
				if($table=='quiz_done_arsip' and $i=="id"){
					continue;
				}	
				...
				if($table=='quiz_done_arsip')
				{
				echo  'REPLACE INTO ' .$table .' (`token`, `uniqid`, `member_id`, `member_code`, `member_class`, `member_fullname`, `start_time`, `start_time_real`, `end_time`, `check_point`, `answer`, `answer_temp`, `schedule_id`, `quiz_id`, `quiz_duration`, `quiz_title_id`, `quiz_code`, `benar`, `salah`, `tidak_jawab`, `kkm`, `score`, `is_done`, `ip_address`, `browser_key`, `acak`, `acak_pilihan`)
				VALUES ('.$values .')';
