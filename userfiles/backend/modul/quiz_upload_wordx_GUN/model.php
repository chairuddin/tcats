...
if ($word_parser==2 or ($word_parser==1 and ($index = $zip->locateName("$htmlname")) !== false))  {
	...
	foreach ($xpath->query($query_path) as $table) {
				...
				foreach ($table->childNodes as $row) {
					...
					if($label=="PERTANYAAN" OR in_array($label,$pilihan_ganda)  OR  $label=="KUNCI"){
						 if($mode_algoritma==1){
	...
	if(count($r_pertanyaan)>0)
	{
		...
		if($_POST['timpa']==1){
		$timpa_soal=$mysql->query("DELETE FROM quiz_detail WHERE quiz_id='$id_soal' $kondisi_paket");
		}
		...
		$sql_r=array();
		$r_exist=$mysql->query("SELECT id FROM quiz_detail WHERE quiz_id='".${"id_soal"}."' AND urutan='".$nomor."' $kondisi_paket");
		if($mysql->numrows($r_exist)>0){
			...
			$sql.=join(",",$sql_r);
			$sql.=" WHERE quiz_id='".${"id_soal"}."' AND urutan='".$nomor."' $kondisi_paket";
		}else{
			$sql="
	...
	{	
		...
		if($_POST['timpa']==1){
		$timpa_soal=$mysql->query("DELETE FROM quiz_essay WHERE quiz_id='$id_soal' $kondisi_paket");
		}
		...
		$sql_r=array();
		$r_exist=$mysql->query("SELECT id FROM quiz_essay WHERE quiz_id='".${"id_soal"}."' AND urutan='".$nomor."' $kondisi_paket");
		if($mysql->numrows($r_exist)>0){
			$sql="
