...
$form->configure(array(
	...
	"action" =>backendurl("$modul/update?id_soal=$id_soal&essay=".$_GET['essay']."&complex=".$_GET['complex']),
...
if($_GET['essay']!=1)
{
		...
		*/
		...
		'.$select_jawaban.'
			...
			$kunci=str_split($d["answer"]);
			$option_pilihan_complex = array_map(function($pil_complex) {
				global $kunci;
					$pil=cleanK($pil_complex);
					ob_start();
					echo "<input type='checkbox' name='r_answer[]' value='".$pil."' ".(in_array($pil,$kunci)?"checked='checked'":"").">$pil&nbsp;&nbsp;&nbsp;";
				return ob_get_clean();
			},$pilihan_komplek);
			
			$option_pilihan_complex=join("",$option_pilihan_complex);
			
			$form->addElement(new Element_HTML('
			<div class="control-group">
			<label for="set_jawaban" class="control-label">Kunci Jawaban<span class="required">* </span></label>
			<div class="controls">
			'.$option_pilihan_complex.'
			</div></div>
			'));
...
}elseif($_GET['complex']==1) {
	$form->addElement(new Element_HTML('<legend>Tambah Komplex</legend>'));
}else{
...
//$col_tambah.='&nbsp;&nbsp;&nbsp;<a href="'.backendurl("$modul/add/?id_soal=$id_soal&complex=1").'" class="btn btn-primary">(+) Komplex</a>';
...
while($d=$mysql->assoc($r))
{
$action_edit=btn_edit(backendurl("$modul/edit/".$d['id']."?id_soal=$id_soal&complex=1"));
$action_delete=btn_delete(backendurl("$modul/del/".$d['id']."?id_soal=$id_soal&complex=1"));	
echo "
<tr>
<td valign=\"top\">$no</td>
";
...
$pilihan_komplek_content = ob_get_clean();

echo <<<END
<div class="card">
		<div class="card-header">
		  <h3 class="card-title">Pilihan Komplek</h3>
		  <div class="card-tools">
			  <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i>
			  </button>
			</div>
