...
if($id=="del_pic_main_block")
{
	...
	$v=$mysql->get1value("SELECT thumbnail FROM menu_block_layout WHERE id='$id_block'");
...
if($id=="move_block")
{
	...
	if($id_layout>0)
	{
		...
		if(count($r_item)>0)
		{
			...
			{
			list($id)=explode(",",$v);
			$q=$mysql->query("UPDATE menu_layout_item SET urutan='$urut',id_layout='$id_layout',position='$position' WHERE id='$id'");	
			$urut++;
			}
		}
	}
...
if($id=="move_main_block")
{
	...
	if($id_block>0 AND $id_menu>0)
	{
		...
		if(count($r_item)>0)
		{
			...
			{
			list($id)=explode(",",$v);
			$q=$mysql->query("UPDATE menu_layout SET urutan='$urut',id_block='$id_block',id_menu='$id_menu' WHERE id='$id'");	
			$urut++;
			}
		}
	}
...
if($id=="save_urutan")
{
	...
	if($id_layout>0)
	{
		...
		if(count($r_item)>0)
		{
			...
			list($id)=explode(",",$v);
			$q=$mysql->query("UPDATE menu_layout_item SET urutan='$urut',id_layout='$id_layout',position='$position' WHERE id='$id'");
			echo "UPDATE menu_layout_item SET urutan='$urut',id_layout='$id_layout',position='$position' WHERE id='$id' <br>";	
			$urut++;
			}
		}
	}
...
if($id=="save_urutan_block")
{
	...
	if(count($r_item)>0)
	{
		...
		$q=$mysql->query("UPDATE menu_layout SET urutan='$urut' WHERE id='$id' and id_menu='$id_menu'");	
		$urut++;
		}
	}
}
if($id=="save_block_wrapper")
{
	...
	if(count($r_item)>0)
	{
		...
		{
		$q=$mysql->query("UPDATE menu_block_layout SET urutan='$urut' WHERE id='$id' and id_menu='$id_menu'");	
		$urut++;
		}
	}
}
if($id=="save_block")
{
	...
	$auto_increment=$mysql->last_auto_increment("menu_layout_item");
	$urutan = getMaxNumber("menu_layout_item", 'urutan'," id_layout='$idlayout' AND position='$position' ")+2;
	$q="
	INSERT INTO menu_layout_item (id,id_layout,name_id,block,urutan,value,position)
	...
	if($r)
	{
		...
		/*
		 echo "<div class=\"kat_item ui-state-default ui-draggable\"><i title='up or down only' class='glyphicon-move'></i><div class=\"kat_nama\"><a href=\"#\" data-pk=\"".$cats->cats[$i]['id']."\"  data-name=\"name_$lang\" class=\"title-cat\">".$cats->cats[$i]['nama']."</a></div>";
           ...
           <a class=\"menu_layout_action-remove\" href=\"" .backendurl("$modul/del/").$cats->cats[$i]['id_menu']."/".$cats->cats[$i]['id']."\"><i class='glyphicon-remove'></i></a>
			</div></div>";
		...
		$djson['block_item']="<div id='item_$auto_increment,$idlayout' class='kat_item block_layout_item ui-state-default ui-draggable $type'>
           ...
           <a href=\"".backendurl("block_".$block."/edit/".$auto_increment."/menu_layout/$idmenu")."\"><i class='icon-cogs'></i></a>
           <a class=\"menu_layout_action-remove\" href=\"" .backendurl("$modul/del_item/").$auto_increment."/".$idmenu."\"><i class='glyphicon-remove'></i></a>
        		</div>
        	<script>
			$('.menu_layout_action-remove').click(function() {
