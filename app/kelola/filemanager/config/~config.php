...
//$cfg_app_url
$base_url=$config['tinyurl'];

 // DON'T TOUCH (base url (only domain) of site (without final /)).
/*JIKA LEVELNYA GURU MAKA DIREKTORI TERBATAS UNTUK DIRINYA SENDIRI*/
$direktory_guru="";
if(!$_SESSION['s_username']){
//header('WWW-Authenticate: Negotiate');
//header('WWW-Authenticate: NTLM', false);
//header("location:/");
die('Sesi telah habis silahkan refresh halaman dan login kembali');	
}
//DIREKTORI GURU
if($_SESSION['s_level']<=2){
	$direktory_guru_1="/".$_SESSION['s_folder_guru'];
	$direktory_guru_2="/".$_SESSION['s_folder_guru']."/dir_".$_SESSION['s_folder_soal'];
	if(file_exists(DIR_IMAGES."/source$direktory_guru_1")==false){
	mkdir(DIR_IMAGES."/source$direktory_guru_1",0755);
	}
	if(file_exists(DIR_IMAGES."/thumbs$direktory_guru_1")==false){
	mkdir(DIR_IMAGES."/thumbs$direktory_guru_1",0755);
	}
	if(file_exists(DIR_IMAGES."/source$direktory_guru_2")==false){
	mkdir(DIR_IMAGES."/source$direktory_guru_2",0755);
	}	
	if(file_exists(DIR_IMAGES."/thumbs$direktory_guru_2")==false){
	mkdir(DIR_IMAGES."/thumbs$direktory_guru_2",0755);
	}
...
//DIREKTORI SOAL
if($_SESSION['s_level']<=2){
	$folder_soal=$_SESSION['s_folder_guru']."/dir_".$_SESSION['s_folder_soal']."/";	
}
/*END JIKA LEVELNYA GURU MAKA DIREKTORI TERBATAS UNTUK DIRINYA SENDIRI*/
$upload_dir ="/userfiles/file/".$_d['dir']."/media/source/$folder_soal"; // path from base_url to base of upload folder (with start and final /)
$current_path ="../../userfiles/file/".$_d['dir']."/media/source/$folder_soal"; // relative path from filemanager folder to upload folder (with final /)
$thumbs_base_path = "../../userfiles/file/".$_d['dir']."/media/thumbs/$folder_soal"; // relative path from filemanager folder to thumbs folder (with final /)
...
//die(DIR_IMAGES."/thumbs$direktory_guru_1");
//die('a');
//--------------------------------------------------------------------------------------------------------
...
$ext_video = array('mov', 'mpeg', 'mp4', '3gp', 'avi', 'mpg','wma',"flv","webm"); //Video 
