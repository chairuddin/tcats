...
$courses=$mysql->sql_get_assoc(" SELECT id,title,if(LENGTH(thumbnail)<=0,'$image_default',concat('$fileurl/app_course/',thumbnail)) thumbnail  FROM app_course "); //WHERE md5(md5(category_id))='".md5($category_default_selected)."'

//$course_sub=$mysql->sql_get_assoc(" SELECT id,title,course_id,if(LENGTH(image)<=0,'$image_default',concat('$fileurl/app_course_sub/',image)) image,is_free FROM app_course_sub ");
