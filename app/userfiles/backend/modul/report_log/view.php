...
$list_report_log=list_report_log(array('user'=>$user,'awal'=>$periode_awal,'akhir'=>$periode_akhir));
$q_user=$mysql->query("SELECT id,nickname FROM user WHERE id <> 1 AND status=1");
$option_user='';
if($q_user and $mysql->num_rows($q_user)) {
	while($d_user=$mysql->fetch_assoc($q_user)) {
		$option_user.='<option value="'.$d_user['id'].'" '.($user==$d_user['id']?'selected="selected"':'').'>'.$d_user['nickname'].'</option>';
	}
}
echo <<<END
<div class="card">
    ...
    <table class="table" style="">
		<tr>
			<th>No</th>
			<th>User</th>
			<th style="text-align:left">Aktifitas</th>
			<th style="text-align:left">Lembaga</th>
			<th style="text-align:leff">Waktu</th>
		</tr>
		{$list_report_log}
