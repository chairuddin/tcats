...
function list_report_log($param=array()) {
	global $mysql,$periode_awal,$periode_akhir;
	$user=$param['user'];
	$awal=$param['awal'];
	$akhir=$param['akhir'];
	...
	l.lembaga_nama
	FROM 
		prospek_followup pf LEFT JOIN prospek_list p ON pf.prospek_list_id=p.id
	LEFT JOIN 
		lembaga l ON l.id=p.lembaga_id 
	LEFT JOIN user u ON u.id=pf.created_by
	WHERE DATE_FORMAT(pf.created_at,'%Y-%m-%d') BETWEEN '$awal' AND '$akhir'  ".($user!=""?" AND pf.created_by=$user ":"")."
	";
	...
	WHERE DATE_FORMAT(p.created_at,'%Y-%m-%d') BETWEEN '$awal' AND '$akhir'  ".($user!=""?" AND p.created_by=$user ":"")."
	...
	FROM 
			invoice i 
	LEFT JOIN 
		lembaga l ON l.id=i.lembaga_id 
	LEFT JOIN user u ON u.id=i.created_by
	WHERE DATE_FORMAT(i.created_at,'%Y-%m-%d') BETWEEN '$awal' AND '$akhir'  ".($user!=""?" AND i.created_by=$user ":"")."
	...
	FROM 
			deal d 
	LEFT JOIN 
		lembaga l ON l.id=d.lembaga_id 
	LEFT JOIN user u ON u.id=d.created_by
	WHERE DATE_FORMAT(d.created_at,'%Y-%m-%d') BETWEEN '$awal' AND '$akhir'  ".($user!=""?" AND d.created_by=$user ":"")."
	...
	WHERE DATE_FORMAT(l.created_at,'%Y-%m-%d') BETWEEN '$awal' AND '$akhir'  ".($user!=""?" AND l.created_by=$user ":"")."
	
	";
	
	
	$sql_join=" SELECT x.* FROM ($sql) x ORDER BY x.created_at ";
	//http://localhost/custom/crm-kuanta/kelola/dashboard/fu_prospek/87?from=prospek/progress_list/22
	$q=$mysql->query($sql_join);
	$response='';
	$coach=array();
	if($q and $mysql->num_rows($q)>0) {
		$no=1;
		while($d=$mysql->fetch_assoc($q)) {
			$response.= '<tr><td>'.$no.'</td><td>'.$d['nickname'].'</td><td>'.$d['activity'].'</td><td>'.$d['lembaga_nama'].'</td><td>'.$d['created_at'].'</td></tr>';
			$no++;
