...
function filter_lembaga() {
	global $mysql;
	echo '<select name="lembaga" id="lembaga" class="form-control">';
		echo '<option value="">Semua</option>';
	echo '</select>';
	
	$url=backendurl("report_prospek/ajax_lembaga");
	echo <<<END
	<script>
	  ...
	  $('#lembaga').select2({
		ajax: {
			...
			processResults: function (data) {
				// Transform the response to match Select2's format
				return {
...
function filter_produk() {
	global $mysql;
	$q=$mysql->query("SELECT kode,nama FROM produk ORDER BY nama");
...
function filter_jenis() {
	global $mysql;
	$q=$mysql->query("SELECT kode,nama FROM produk_jenis ORDER BY nama");
	
	echo '<select name="jenis" class="form-control">';
...
function list_report_prospek($periode=array()) {
	global $mysql;
	$awal=$periode['awal'];
	$akhir=$periode['akhir'];
	$q=$mysql->query(" SELECT l.lembaga_nama,u.nickname sales,pl.created_at,pr.harga,pr.nama produk_nama,pl.status,pj.nama jenis FROM prospek_list pl 
	LEFT JOIN lembaga l ON l.id=pl.lembaga_id
	LEFT JOIN user u ON u.id=pl.created_by
	LEFT JOIN prospek p ON p.id=pl.prospek_id
	LEFT JOIN produk pr ON pr.id=p.produk_id
	LEFT JOIN produk_jenis PJ ON pj.id=pr.jenis
	WHERE
	DATE_FORMAT(pl.created_at,'%Y-%m-%d') BETWEEN '$awal' AND '$akhir' 
	ORDER BY l.lembaga_nama
	");
	$response='';
	$coach=array();
	if($q and $mysql->num_rows($q)>0) {
		...
		while($d=$mysql->fetch_assoc($q)) {
			...
			$response.= '<tr><td>'.$no.'</td><td>'.$d['lembaga_nama'].'</td><td>'.$d['jenis'].'</td><td style="text-align:left">'.$d['produk_nama'].'</td><td   style="text-align:left">'.$d['sales'].'</td><td   style="text-align:center">'.tgl_indo_short(date("Y-m-d",strtotime($d['created_at']))).'</td><td   style="text-align:center">'.badge_color($d['status']).'</td><td   style="text-align:center">'.currency($d['harga']).'</td></tr>';
			$no++;
