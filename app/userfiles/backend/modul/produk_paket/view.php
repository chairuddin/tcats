...
foreach($list_produk as $i => $v) {
	...
	<div class="form-check">
		<input type="checkbox" class="list_produk form-check-input"  data-harga="'.$v['harga'].'" onclick="hitung_total_harga()" name="list_produk[]" id="produk'.$i.'" value="'.$i.'" '.($produk_terpilih[$i]>0?'checked="checked"':'').'/>
		<label class="form-check-label" onclick="hitung_total_harga()" for="produk'.$i.'">&nbsp;&nbsp;&nbsp;'.$v['nama'].' ('.currency($v['harga']).')</label>
	</div>';
}
$form_list_produk.='';
...
if($action=="data") {
	...
	$sql=" SELECT b.id,b.kode,b.nama,harga FROM $table b  ";
	
	$sql.=" WHERE is_paket=1 ";
