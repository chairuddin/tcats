...
function ringkasan_pelanggan($periode=array()) {
	...
	$data['konversi_pelanggan']=konversi_pelanggan($periode);
...
function konversi_pelanggan($periode=array()) {
	/*
	Konversi pelanggan/ prospek itu persentase pelanggan yang sudah dapat penawaran dari kita. Contoh kita punya database 10 sekolah. Lalu baru ada 1 sekolah itu ditawari 2 produk. persentase seharusnya adalah 10% bukan 20%. Dan tidak mungkin persentasenya lebih dari 100%
	 */
	//database baru yang jadi pelanggan berapa %
	global $mysql;
	//$awal=$periode['awal'];
	//$akhir=$periode['akhir'];
	$total_lembaga=$mysql->get1value("SELECT count(id) FROM lembaga");
	$total_prospek=$mysql->get1value(" SELECT count(distinct lembaga_id) FROM prospek_list ");
	$konversi=($total_prospek/$total_lembaga)*100;
	return $konversi;	
}
...
function pelanggan_baru($periode=array()) {
	...
	$awal=$periode['awal'];
	$akhir=$periode['akhir'];
	$last_2_years = date('Y-m-d', strtotime('-2 years', strtotime(date("Y-m-d"))));
	$total=$mysql->get1value(" SELECT sum(x.jml) FROM (
		SELECT lembaga_id,count(lembaga_id) jml FROM deal WHERE tanggal>'$last_2_years' 
		) x WHERE x.jml<=1
		 ");
