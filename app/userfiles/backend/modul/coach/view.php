...
$option_is_internal=array();
$option_is_internal[]='Pilih Status';
$option_is_internal[1]='Internal';
$option_is_internal[2]='External';
$form_nama=$form->element_Textbox("Nama","nama");
$form_hp=$form->element_Textbox("Nomor HP.","hp",array('autocomplete'=>'off'));
$form_id_trainer=$form->element_Textbox("ID Trainer","id_trainer",array('placeholder'=>'Biarkan kosong untuk generate otomatis'));
$form_expired=$form->element_Textbox("Expired","expired",array('class'=>'min-date'));
$form_is_internal=$form->element_Select("Status Trainer","is_internal",$option_is_internal);
...
if($action=="data") {
	...
	$column_order = array('b.id','b.nama','hp','b.id_trainer','b.expired');
	$column_search = array('b.nama','hp','b.id_trainer','b.expired');
	...
	$sql=" SELECT b.id,b.nama,b.id_trainer,b.expired,b.hp FROM $modul b  ";
	...
	while($d = $mysql->fetch_assoc($result)) {
		...
		if($daftar_hak[$modul]['edit']==1) {
			$action_edit='<a  title="Edit Coach" href="'.backendurl("$modul/edit/".$d['id']).'"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
		}
