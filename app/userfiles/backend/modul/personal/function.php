...
function find_lembaga_jenjang_by_nama($arr=array(),$keyword="") {
    foreach($arr as $key => $val) {
...
function find_pekerjaan_by_nama($keyword_pekerjaan="") {
	$list_pekerjaan=list_pekerjaan();
	foreach($list_pekerjaan as $val =>$pekerjaan) {
		if(strtolower(cleanInput($keyword_pekerjaan,'alpha'))==strtolower(cleanInput($pekerjaan,'alpha'))) {
            return $val;
        }
	}
	return 0;
