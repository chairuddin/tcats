...
function insert_customer($nik) {
	global $mysql,$id_cabang,$admin_id,$hari_ini;
		$is_exist=$mysql->get1value("SELECT count(id) ketemu FROM master_customer WHERE nik='".cleanInput($nik)."'");
...
function generate_urut_invoice($prefik="",$digit=3,$table="invoice"){
	...
	$getmaxnumber=$mysql->get1value(" SELECT IFNULL((right(max(urut),$digit)),0) nomor FROM $table WHERE LEFT(urut,$prefik_length)='$prefik' ");
	$temp_max=$getmaxnumber+1;
	if(strlen($temp_max)>$digit){
		
		$digit+=(strlen($temp_max)-$digit);	
		$getmaxnumber=$mysql->get1value(" SELECT IFNULL(max(right(urut,$digit)),0) nomor FROM $table WHERE LEFT(urut,$prefik_length)='$prefik' ");
