...
$validation->set_validation(array('var'=>'title','label'=>'Judul Project'))->minlength(1)->required();
...
if($action=="save" or $action=="update") {
	...
	if($validation->valid()){
		...
		*/
		...
		sweetalert2($type="warning",$msg=($action=="update"?"Update":"Tambah")." Proyek gagal, data tidak valid",backendurl("$modul".($action=="update"?"/edit/$id":"/add")));
	}
	
	if($q){
		sweetalert2($type="success",$msg=($action=="update"?"Update":"Tambah")." Proyek berhasil",backendurl("$modul"));
	} else {
		sweetalert2($type="warning",$msg=($action=="update"?"Update":"Tambah")." Proyek gagal. ",backendurl("$modul".($action=="update"?"/edit/$id":"/add")));
	}
...
$r=$mysql->query($sql);
	...
	if($r){
		sweetalert2($type="success",$msg="Hapus Proyek berhasil",backendurl("$modul"));
	} else {
		sweetalert2($type="warning",$msg="Proyek tidak bisa dihapus",backendurl("$modul"));
	}
