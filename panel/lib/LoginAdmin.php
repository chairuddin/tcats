...
Class Login 
{
    ...
    function dologin($username="",$password="") {
        ...
        $q=$this->db->query("SELECT id,username,email FROM user WHERE (md5(username)='$username' OR md5(email)='$username' ) AND password='$password' AND status=1");
    ...
    function search_user_by_id($id) {
        list($data)=$this->db->query_data("SELECT * FROM user WHERE id='$id'");
        return $data;
    }
    function search_user_by_token($token) {
     
        list($data)=$this->db->query_data("SELECT * FROM user WHERE token='$token'");
        return $data;
    }
	function search_username($username) {
		list($data)=$this->db->query_data("SELECT * FROM user WHERE username='$username'");
		return $data;
	} 
	function search_username_by_email($email) {
		list($data)=$this->db->query_data(" SELECT * FROM user WHERE email='$email' ");
		return $data;
	} 
	...
	function request_reset($email,$from='web') {
		...
		if($data['id']>0) {
			...
			$update_kode=$this->db->query("UPDATE user SET kode_reset='$stamp',request_reset='$waktu',reset_from='$from' WHERE email='$email'");
	...
	function send_email_reset($fullname,$stamp,$email) {
		...
		$value_code_reset=backendurl("app_reset_password/$stamp");
	...
	function is_request_reset($stamp) {
		...
		$cek=$this->db->query("SELECT * FROM user WHERE kode_reset='$stamp' AND request_reset < DATE_ADD('$hari_ini', INTERVAL 1 DAY) ");
	...
	function renew_password($password,$email) {
		$password=md5(sha1($password));
		$update_password=$this->db->query("UPDATE user SET kode_reset='',request_reset='',password='$password' WHERE email='$email'");
		return $update_password;
	}
    function set_token($token,$id) {
      
        $update_token=$this->db->query("UPDATE user SET token='$token',WHERE id='$id'");
        return $update_token;
    }
